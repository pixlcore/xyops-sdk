...
const api = new Proxy({
	...
	async sendRequest(method, req, opts) {
		...
		url = url.replace(/\/$/, '') + '/api/' + api_namespace + '/' + method + '/v1';
		
		// if method is get/search, use query string instead of POST
		if (method.match(/^(get_|search_|stream_|config)/) || opts.download) {
		...
		if (method.match(/^(stream_)/)) {
			...
			if (!iterator) return this.doError(method + ": Missing iterator function for stream.");
		...
		if (opts.files) {
			...
			catch (err) { return this.doError(method + ": " + err); }
			...
			if (!resp.resp.statusCode.toString().match(this.request.successMatch)) {
				return this.doError(method + ": " + "HTTP " + resp.resp.statusCode + " " + resp.resp.statusMessage + ": " + url);
			}
		}
		else {
			// send using JSON API
		...
		if (resp.data && resp.data.code) {
			resp.err = method + ": " + resp.data.description;
